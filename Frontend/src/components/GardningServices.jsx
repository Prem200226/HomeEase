...
function DisplayGardening() {
    const navigate = useNavigate();
    const [services, setServices] = useState([]);
    const [isLoading, setIsLoading] = useState(true);
    const category = 'Home Spa';
    ...
    return (
        <Container className="my-4">
            <h2 className="mb-4 text-center">Home Spa Services</h2>
            <p className="mb-5 text-center text-muted" style={{ maxWidth: 600, margin: '0 auto' }}>
                Explore our top-rated Home Spa services. Book a professional for all your Beauty maintenance.
            </p>
            {isLoading ? (
                    ...
                    {services.map((service, idx) => (
                        <Col key={service.service_id || service.id || idx} md={4} sm={6} xs={12}>
                            <Card style={{ width: '100%', minHeight: 420, boxShadow: '0 2px 8px #eee' }}>
                                <Card.Img 
                                    variant="top" 
                                    src={service.image_url || service.image || '/placeholder.png'} 
...
export default DisplayGardening;
